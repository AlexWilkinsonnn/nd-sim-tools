...
#include "CAF_EhadCorr.h"

CAF_EhadCorr::CAF_EhadCorr( std::string filename, bool isGas )
{
  ...
  cafMVA->Branch( "CorrEhad_veto", &CorrEhad_veto, "CorrEhad_veto/D" );
...
CAF_EhadCorr::~CAF_EhadCorr() {}

void CAF_EhadCorr::fill()
{
  cafMVA->Fill();
  genie->Fill();
}

void CAF_EhadCorr::Print()
{
  printf( "Event %d:\n", event );
...
void CAF_EhadCorr::fillPOT()
{
  printf( "Filling metadata\n" );
  cafPOT->Fill();
}

void CAF_EhadCorr::write()
{
  cafFile->cd();
  cafMVA->Write();
  cafPOT->Write();
...
void CAF_EhadCorr::addRWbranch( int parId, std::string name, std::string wgt_var, std::vector<double> &vars )
{
