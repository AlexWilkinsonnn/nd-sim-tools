...
void recoMuonTracker( CAF_EhadCorr &caf, params &par )
{
...
void recoMuonLAr( CAF_EhadCorr &caf, params &par )
{
...
// Fill reco variables for muon reconstructed in magnetized tracker
void recoMuonECAL( CAF_EhadCorr &caf, params &par )
{
  // range-based KE
  double ke = caf.LepE - mmu;
...
void recoElectron( CAF_EhadCorr &caf, params &par )
{
...
// main loop function
void loop( CAF_EhadCorr &caf, params &par, TTree * tree, TTree* tree_resim, TTree * gtree, std::string fhicl_filename )
{
  ...
  for( int ii = par.first; ii < N; ++ii ) {
	...
	if((caf.vtx_x < 310.) && (caf.vtx_x > -310.) && (caf.vtx_y < 550.) && (caf.vtx_y > -550.) && (caf.vtx_z < 1244.) && (caf.vtx_z > 50.) && ((caf.muon_tracker == 1) || (caf.muon_contained == 1)) && (caf.reco_numu == 1) && (((caf.reco_q == -1) && (caf.neutrinoPDG > 0)) || ((caf.reco_q == 1) && (caf.neutrinoPDG < 0)))) {
        caf.fill();
	}
  }
...
int main( int argc, char const *argv[] )
{
  ...
  CAF_EhadCorr caf( outfile, par.IsGasTPC );
