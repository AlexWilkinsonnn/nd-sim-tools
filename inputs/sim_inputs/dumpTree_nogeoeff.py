...
def loop( evt, tgeo, tout, muoncontained_ids_file ):
    ...
    for ient in range(N):
        ...
        for ivtx,vertex in enumerate(event.Primaries):
            ...
            if abs(t_lepPdg[0]) == 13:
                ...
                if "LArActive" in endVolName:
                    t_muonReco[0] = 1 # contained
                    if muoncontained_ids_file:
                        with open(muoncontained_ids_file, 'a+') as f:
                            f.write(str(event.EventId) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        help="File to write eventIDs where the muon is contained in the LAr", default=""
    )
    ...
    loop( events, tgeo, tout, args.muoncontained_ids_file )
