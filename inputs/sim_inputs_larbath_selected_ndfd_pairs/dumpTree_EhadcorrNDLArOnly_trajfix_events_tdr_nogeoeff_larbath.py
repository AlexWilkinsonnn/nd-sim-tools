...
def loop( evt, tgeo, tout ):
    ...
    for ient in range(N):
        ...
        for ivtx,vertex in enumerate(event.Primaries):
            ...
            trk_calo_corr = [0. for i in range(nfsp)]
            corr_trk_calo = [0. for i in range(nfsp)]
            gamma_energy = {}
            for g in gamma_tids:
                gamma_energy[g] = 0.
            for hit in hits:
            ...
            corr_total_energy = 0.0
            corr_collar_energy = 0.0
            t_CorrhadP[0] = 0.0
            t_CorrhadN[0] = 0.0
            t_CorrhadPip[0] = 0.0
            t_CorrhadPim[0] = 0.0
            t_CorrhadPi0[0] = 0.0
            t_CorrhadOther[0] = 0.0
            ...
            for hit in inactive_hits:
                ...
                if hit.PrimaryId != ileptraj: # here we do want to associate stuff to the lepton
                    ...
                    pdg = traj_to_pdg[traj]
                    if pdg in [11, -11, 13, -13]: continue # lepton
                    elif pdg == 2212: t_TotCorrP[0] += Edep_corr
                    elif pdg == 2112: t_TotCorrN[0] += Edep_corr
                    elif pdg == 211: t_TotCorrPip[0] += Edep_corr
                    elif pdg == -211: t_TotCorrPim[0] += Edep_corr
                    elif pdg == 111: t_TotCorrPi0[0] += Edep_corr
                    else: t_TotCorrOther[0] += Edep_corr
                    
            # Perform the checks to make sure there are no negative 
            # energies, and assign variables
            if(total_corr + total_energy < 0.0):
              t_CorrhadTot[0] = total_energy
            else:
              t_CorrhadTot[0] = total_energy + total_corr
            t_hadTot[0] = total_energy
            t_TotCorr[0] = total_corr
            if(total_corr_collar + collar_energy < 0.0):
              t_CorrhadCollar[0] = collar_energy
            else:
              t_CorrhadCollar[0] = collar_energy + total_corr_collar
            t_hadCollar[0] = collar_energy
            t_TotCorrCollar[0] = total_corr_collar
            if(t_TotCorrP[0] + t_hadP[0] < 0.0):
              t_CorrhadP[0] += t_hadP[0]
            else:
              t_CorrhadP[0] = t_hadP[0] + t_TotCorrP[0]
            if(t_TotCorrN[0] + t_hadN[0] < 0.0):
              t_CorrhadN[0] += t_hadN[0]
            else:
              t_CorrhadN[0] = t_hadN[0] + t_TotCorrN[0]
            if(t_TotCorrPip[0] + t_hadPip[0] < 0.0):
              t_CorrhadPip[0] += t_hadPip[0]
            else:
              t_CorrhadPip[0] = t_hadPip[0] + t_TotCorrPip[0]
            if(t_TotCorrPim[0] + t_hadPim[0] < 0.0):
              t_CorrhadPim[0] += t_hadPim[0]
            else:
              t_CorrhadPim[0] = t_hadPim[0] + t_TotCorrPim[0]
            if(t_TotCorrPi0[0] + t_hadPi0[0] < 0.0):
              t_CorrhadPi0[0] += t_hadPi0[0]
            else:
              t_CorrhadPi0[0] = t_hadPi0[0] + t_TotCorrPi0[0]
            if(t_TotCorrOther[0] + t_hadOther[0] < 0.0):
              t_CorrhadOther[0] += t_hadOther[0]
            else:
              t_CorrhadOther[0] = t_hadOther[0] + t_TotCorrOther[0]

            for i in range(nfsp):
                if(trk_calo_corr[i] + trk_calo[i] < 0.0):
                  corr_trk_calo[i] = trk_calo[i]
                else:
                  corr_trk_calo[i] = trk_calo[i] + trk_calo_corr[i]
                ...
                t_fsTrkCaloCorr[i] = trk_calo_corr[i]
                t_CorrfsTrkCalo[i] = corr_trk_calo[i]
...
if __name__ == "__main__":
    ...
    t_TotCorrP = array('f', [0.] )
    tout.Branch('TotCorrP', t_TotCorrP, 'TotCorrP/F')
    t_TotCorrN = array('f', [0.] )
    tout.Branch('TotCorrN', t_TotCorrN, 'TotCorrN/F')
    t_TotCorrPip = array('f', [0.] )
    tout.Branch('TotCorrPip', t_TotCorrPip, 'TotCorrPip/F')
    t_TotCorrPim = array('f', [0.] )
    tout.Branch('TotCorrPim', t_TotCorrPim, 'TotCorrPim/F')
    t_TotCorrPi0 = array('f', [0.] )
    tout.Branch('TotCorrPi0', t_TotCorrPi0, 'TotCorrPi0/F')
    t_TotCorrOther = array('f', [0.] )
    tout.Branch('TotCorrOther', t_TotCorrOther, 'TotCorrOther/F')
    t_nFS = array('i',[0])
    ...
    t_CorrfsTrkCalo = array('f',100*[0.])
    tout.Branch('CorrfsTrkCalo',t_CorrfsTrkCalo,'fsTrkCalo[nFS]/F')
    t_fsTrkCaloCorr = array('f',100*[0.])
    tout.Branch('fsTrkCaloCorr',t_fsTrkCaloCorr,'fsTrkCaloCorr[nFS]/F') 
