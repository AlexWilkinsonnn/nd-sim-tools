...
# Activate all regions of the detector
IFS_tmp=$IFS
IFS=" "
lines_added=0
...
do
	...
	current_n_lines=$((lines + lines_added))
	pct=$(echo "scale=4; $next_line_no / $current_n_lines * 100" | bc | cut -d 0 -f 1)
	echo "Progress: Line $next_line_no / $current_n_lines (${pct} %)"
