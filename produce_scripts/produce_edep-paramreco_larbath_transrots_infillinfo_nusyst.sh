...
# Includes variables with infill information and xsec weights from
# nusystematics
################################################################################
# Options
...
python dumpTree_nogeoeff_larbath_infillinfo.py --infile edep_larbath.${RNDSEED}.root \
                                               --edepsim_geometry ${GEOMETRY_ND_DUMMY_EDEP} \
                                               --outfile edep_dump_larbath_nd.${RNDSEED}.root

echo "Running makeCAF"
cd $ND_CAFMAKER_DIR
./makeCAFInfillInfoNuSyst --infile ../edep_dump_larbath_nd.${RNDSEED}.root \
