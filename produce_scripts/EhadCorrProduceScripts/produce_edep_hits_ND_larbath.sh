...
GENIE_OUTPATH="/exp/dune/data/users/colweber/larbath_ndfd_pairs/tdr_sample/FHC/genie"
EDEP_ND_ACT_OUTPATH="/exp/dune/data/users/colweber/larbath_ndfd_pairs/tdr_sample/FHC/edep/ND_ACT"
EDEP_LAr_ACT_OUTPATH="/exp/dune/data/users/colweber/larbath_ndfd_pairs/tdr_sample/FHC/edep/LAr_ACT"
EDEP_DUMP_ND_ACT_OUTPATH="/exp/dune/data/users/colweber/larbath_ndfd_pairs/tdr_sample/FHC/edep_dump/ND_ACT"
EDEP_DUMP_LAr_ACT_OUTPATH="/exp/dune/data/users/colweber/larbath_ndfd_pairs/tdr_sample/FHC/edep_dump/LAr_ACT"
...
GEOMETRY_ND_ACTIVE="AllAct_MPD_SPY_LAr.gdml"
...
ALL_ACT_GEOMETRY_ND="AllAct_$GEOMETRY_ND"
if [ ! -f $ALL_LAr_GEOMETRY_ND ];
then
	bash activate_all_lar_geometry_nd.sh -i $GEOMETRY_ND -o $ALL_LAr_GEOMETRY_ND
fi
if [ ! -f $ALL_ACT_GEOMETRY_ND ];
then
	bash activate_all_volume_geometry_nd.sh -i $GEOMETRY_ND -o $ALL_ACT_GEOMETRY_ND
fi
...
python dumpTree_hits.py --infile_edepsim edep_larbath.${RNDSEED}.root \
...
ls -lrth
python dumpTree_hits.py --infile_edepsim edep_ND.${RNDSEED}.root \
                                        --edepsim_geometry edep_ND.${RNDSEED}.root \
                                        --outfile edep_dump_ND_nd.${RNDSEED}.root

echo "Copying files do /exp/dune/data..."
if [ "$SAVE_GENIE" = true ]; then
  cp ${MODE}.${RNDSEED}.ghep.root ${GENIE_OUTPATH}/${HORN}.${RNDSEED}.ghep.root
fi
if [ "$SAVE_GTRAC" = true ]; then
  cp ${MODE}.${RNDSEED}.gtrac.root ${GENIE_OUTPATH}/${HORN}.${RNDSEED}.gtrac.root
fi
if [ "$SAVE_EDEP" = true ]; then
  cp edep_larbath.${RNDSEED}.root ${EDEP_LAr_ACT_OUTPATH}/${HORN}.${RNDSEED}.edep_larbath.root
  cp edep_ND.${RNDSEED}.root ${EDEP_ND_ACT_OUTPATH}/${HORN}.${RNDSEED}.edep_ND.root
fi
if [ "$SAVE_EDEP_MAKECAF" = true ]; then
  cp edep_dump_larbath_nd.${RNDSEED}.root ${EDEP_DUMP_LAr_ACT_OUTPATH}/${HORN}.${RNDSEED}.edep_dump_larbath_nd.root
  cp edep_dump_ND_nd.${RNDSEED}.root ${EDEP_DUMP_ND_ACT_OUTPATH}/${HORN}.${RNDSEED}.edep_dump_ND_nd.root
fi
