...
PAIR_H5_OUTPUT="/pnfs/dune/scratch/users/colweber/larbath_ndfd_pairs/tdr_sample/FHC/Ehad_corr_pair_allinfo_h5"
...
python dumpTree_Ehadcorr_events_tdr_nogeoeff_larbath.py \
			--infile_edepsim edep_larbath.${RNDSEED}.root \
            --edepsim_geometry edep_ND.${RNDSEED}.root \
            --outfile edep_dump_larbath_nd_events.${RNDSEED}.root

python dumpTree_Ehadcorr_parts_tdr_nogeoeff_larbath.py --infile_edepsim edep_larbath.${RNDSEED}.root --edepsim_geometry edep_ND.${RNDSEED}.root --outfile edep_dump_larbath_nd_parts.${RNDSEED}.root
...
python dumpTree_Ehadcorr_events_tdr_nogeoeff_larbath.py \
    ...
    --edepsim_geometry edep_ND.${RNDSEED}.root \
    --outfile edep_dump_ND_nd_events.${RNDSEED}.root

python dumpTree_Ehadcorr_parts_tdr_nogeoeff_larbath.py --infile_edepsim edep_ND.${RNDSEED}.root --edepsim_geometry edep_ND.${RNDSEED}.root --outfile edep_dump_ND_nd_parts.${RNDSEED}.root

echo "LS-ing inputs post dumpTree on ND, pre makeCAF"
ls -lrth
echo "Running makeCAF"
cd $ND_CAFMAKER_DIR
./makeCAF_resim-muon-EhadCorr \
	--infile ../edep_dump_larbath_nd_events.${RNDSEED}.root \
	--infile_resim ../edep_dump_ND_nd_events.${RNDSEED}.root \
...
# 5-16-25 found bug in energy correction where correction was being applied to all hits (including those outside NDLAr). The next block examines what happens if we only correct what's in NDLAr. 

python dumpTree_EhadcorrNDLArOnly_events_tdr_nogeoeff_larbath.py --infile_edepsim edep_larbath.${RNDSEED}.root --edepsim_geometry edep_ND.${RNDSEED}.root --outfile edep_dump_larbath_nd_events_EhadCorrNDLArOnly.${RNDSEED}.root

echo "LS-ing inputs post dumpTree on LAr, pre dumpTree on ND"
ls -lrth
python dumpTree_EhadcorrNDLArOnly_events_tdr_nogeoeff_larbath.py --infile_edepsim edep_ND.${RNDSEED}.root --edepsim_geometry edep_ND.${RNDSEED}.root --outfile edep_dump_ND_nd_events_EhadCorrNDLArOnly.${RNDSEED}.root
